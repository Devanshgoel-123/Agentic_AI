...
const useSendRemoveLiquidityUniV3 = ({
  token0,
  token1,
  poolName,
  poolChainId,
  ...
  const sendRemoveLiquidityTransaction = async () => {
    try {
      ...
      const data = encodeFunctionData({
        abi: useUniV3Store.getState().withdrawContractConfig?.abi as Abi,
        functionName:
          (useUniV3Store.getState().withdrawContractConfig
            ?.functionName as string) ?? undefined,
        args: useUniV3Store.getState().withdrawContractConfig?.args,
      });
      const result = await estimateGas(config, {
        chainId: poolChainId as 1 | 56 | 137 | 7000 | undefined,
        data,
        to: useUniV3Store.getState().withdrawContractConfig
          ?.address as `0x${string}`,
        value: useUniV3Store.getState().withdrawContractConfig?.value
          ? BigInt(useUniV3Store.getState().withdrawContractConfig?.value)
          : undefined,
      });
      /**
      ...
      setEnableQuery(() => true);
      writeContract({
        address: useUniV3Store.getState().withdrawContractConfig
          ?.address as `0x${string}`,
        abi: useUniV3Store.getState().withdrawContractConfig?.abi as
          | Abi
          | readonly unknown[],
        functionName: useUniV3Store.getState().withdrawContractConfig
          ?.functionName as string,
        args: useUniV3Store.getState().withdrawContractConfig?.args as
          | readonly any[]
          | (any[] & readonly any[]),
        gas: result
...
export default useSendRemoveLiquidityUniV3;
