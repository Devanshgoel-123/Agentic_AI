...
const useSendCollectFeeTransaction = ({ poolChainId, callBackFn }: Props) => {
  ...
  const sendCollectFeeTransaction = async () => {
    try {
      ...
      const data = encodeFunctionData({
        abi: useUniV3Store.getState().collectFeeContractConfig?.abi as Abi,
        functionName:
          (useUniV3Store.getState().collectFeeContractConfig
            ?.functionName as string) ?? undefined,
        args: useUniV3Store.getState().collectFeeContractConfig?.args,
      });
      const result = await estimateGas(config, {
        chainId: poolChainId as 1 | 56 | 137 | 7000 | undefined,
        data,
        to: useUniV3Store.getState().collectFeeContractConfig
          ?.address as `0x${string}`,
        value: useUniV3Store.getState().collectFeeContractConfig?.value
          ? BigInt(useUniV3Store.getState().collectFeeContractConfig?.value)
          : undefined,
      });
      /**
      ...
      setEnableQuery(() => true);
      writeContract({
        address: useUniV3Store.getState().collectFeeContractConfig
          ?.address as `0x${string}`,
        abi: useUniV3Store.getState().collectFeeContractConfig?.abi as
          | Abi
          | readonly unknown[],
        functionName: useUniV3Store.getState().collectFeeContractConfig
          ?.functionName as string,
        args: useUniV3Store.getState().collectFeeContractConfig?.args as
          | readonly any[]
          | (any[] & readonly any[]),
        gas: result
...
export default useSendCollectFeeTransaction;
