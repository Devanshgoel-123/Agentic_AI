...
const useSendIncreaseLiquidityTransaction = ({
  token0,
  token1,
  poolName,
  poolChainId,
  ...
  const sendIncreaseLiquidityTransaction = async () => {
    try {
      console.log({
        abi: useUniV3Store.getState().increaseLiquidityContractConfig
          ?.abi as Abi,
        functionName:
          (useUniV3Store.getState().increaseLiquidityContractConfig
            ?.functionName as string) ?? undefined,
        args: useUniV3Store.getState().increaseLiquidityContractConfig?.args,
      });
      /**
      ...
      const result = await estimateGas(config, {
        ...
        data,
        to: useUniV3Store.getState().increaseLiquidityContractConfig
          ?.address as `0x${string}`,
        value: useUniV3Store.getState().increaseLiquidityContractConfig?.value
          ? BigInt(
              useUniV3Store.getState().increaseLiquidityContractConfig?.value
            )
          : undefined,
      });
      /**
      ...
      setEnableQuery(() => true);
      writeContract({
        address: useUniV3Store.getState().increaseLiquidityContractConfig
          ?.address as `0x${string}`,
        abi: useUniV3Store.getState().increaseLiquidityContractConfig?.abi as
          | Abi
          | readonly unknown[],
        functionName: useUniV3Store.getState().increaseLiquidityContractConfig
          ?.functionName as string,
        args: useUniV3Store.getState().increaseLiquidityContractConfig?.args as
          | readonly any[]
          | (any[] & readonly any[]),
        gas: result
...
export default useSendIncreaseLiquidityTransaction;
